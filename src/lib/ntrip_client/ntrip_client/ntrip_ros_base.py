...
class NTRIPRosBase(Node):
  def __init__(self, name):
    ...
    self._rtcm_frame_id = self.get_parameter('rtcm_frame_id').get_parameter_value().string_value

    # Determine the type of RTCM message that will be published
    self.declare_parameter('rtcm_message_package', _MAVROS_MSGS_NAME)
    rtcm_message_package = self.get_parameter('rtcm_message_package').get_parameter_value().string_value
    ...
    self.declare_parameter('reconnect_attempt_max', NTRIPBase.DEFAULT_RECONNECT_ATTEMPT_MAX)
    self.declare_parameter('reconnect_attempt_wait_seconds', NTRIPBase.DEFAULT_RECONNECT_ATEMPT_WAIT_SECONDS)
    self.declare_parameter('reconnect_backoff_base', NTRIPBase.DEFAULT_RECONNECT_BACKOFF_BASE)
    self.declare_parameter('reconnect_backoff_max_seconds', NTRIPBase.DEFAULT_RECONNECT_BACKOFF_MAX_SECONDS)

    self._nmea_max_length = self.get_parameter('nmea_max_length').get_parameter_value().integer_value
    self._nmea_min_length = self.get_parameter('nmea_min_length').get_parameter_value().integer_value
    self._reconnect_attempt_max = self.get_parameter('reconnect_attempt_max').get_parameter_value().integer_value
    self._reconnect_attempt_wait_seconds = self.get_parameter('reconnect_attempt_wait_seconds').get_parameter_value().integer_value
    self._reconnect_backoff_base = self.get_parameter('reconnect_backoff_base').get_parameter_value().double_value
    self._reconnect_backoff_max_seconds = self.get_parameter('reconnect_backoff_max_seconds').get_parameter_value().integer_value
  ...
  def run(self):
    ...
    try:
      while rclpy.ok() and not self._client._shutdown:
