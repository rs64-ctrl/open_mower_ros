...
public:
    CoveragePlannerNode() : Node("slic3r_coverage_planner") {
        this->declare_parameter("visualize_plan", true);
        ...
        if (visualize_plan_) {
            marker_array_publisher_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
                    "slic3r_coverage_planner/path_marker_array", rclcpp::QoS(100).transient_local());
        }

        plan_path_srv_ = this->create_service<slic3r_coverage_planner::srv::PlanPath>(
                "slic3r_coverage_planner/plan_path",
                std::bind(&CoveragePlannerNode::planPath, this, std::placeholders::_1, std::placeholders::_2));
    }

private:
    bool visualize_plan_;
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr marker_array_publisher_;
    rclcpp::Service<slic3r_coverage_planner::srv::PlanPath>::SharedPtr plan_path_srv_;

    void
    createMarkers(const slic3r_coverage_planner::srv::PlanPath::Request::SharedPtr &planning_request,
                  const slic3r_coverage_planner::srv::PlanPath::Response::SharedPtr &planning_result,
    ...
    static void traverse_from_left(std::vector<PerimeterGeneratorLoop> &contours, std::vector<Polygons> &line_groups) {
    ...
    static void traverse_from_right(std::vector<PerimeterGeneratorLoop> &contours, std::vector<Polygons> &line_groups) {
    ...
    slic3r_coverage_planner::msg::Path determinePathForOutline(std_msgs::msg::Header &header, Slic3r::Polygon &outline_poly, Slic3r::Polygons &group, bool isObstacle, Point *areaLastPoint) {
        ...
        for (int i = 0; i < (int)group.size(); i++) {
            ...
            if (points.size() < 2) {
                RCLCPP_INFO(this->get_logger(), "Skipping single dot");
                continue;
            }
            RCLCPP_INFO_STREAM(this->get_logger(), "Got " << points.size() << " points");
    ...
    void planPath(const slic3r_coverage_planner::srv::PlanPath::Request::SharedPtr req,
                  slic3r_coverage_planner::srv::PlanPath::Response::SharedPtr res) {
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "generating " << loops << " outlines");
        ...
        if (!req->skip_fill) {
            ...
            for (auto &poly: expp) {
                ...
                RCLCPP_INFO_STREAM(this->get_logger(), "Starting Fill. Poly size:" << surface.expolygon.contour.points.size());
                ...
                RCLCPP_INFO_STREAM(this->get_logger(), "Fill Complete. Polyline count: " << lines.size());
                for (int i = 0; i < (int)lines.size(); i++) {
                    RCLCPP_INFO_STREAM(this->get_logger(), "Polyline " << i << " has point count: " << lines[i].points.size());
                }
            }
        }
        ...
        if (!req->skip_fill) {
            for (int i = 0; i < (int)fill_lines.size(); i++) {
                ...
                RCLCPP_INFO_STREAM(this->get_logger(), "Got " << equally_spaced_points.size() << " points");
